...
SCORE_LEAD_PROMPT        → 리드 최종 점수 (1–10, 아웃리치 분기 결정)
                              ↳ FINAL SCORE ≥ 6 → 아웃리치 진행
                              ↳ FINAL SCORE < 6 → 리포트 저장만
```
...
```
FINAL SCORE = (C1 × 0.35) + (C2 × 0.05) + (C3 × 0.10) + (C4 × 0.10)
            + (C5 × 0.05) + (C6 × 0.05) + (C7 × 0.30)
```
...
|---|---|---|
| ≥ 6.0 | **Qualified** | 아웃리치 리포트 + 이메일 + 인터뷰 스크립트 생성 |
| < 6.0 | **Not Qualified** | 리포트 저장 + CRM 업데이트만 |
...
| 3 | 기술 관련 정보 없음 (데이터 부족) |
| 1 | 완전 커스텀 엔터프라이즈 시스템 사용으로 **명확히 확인됨** |
...
- 공식: `(C1×0.35) + (C2×0.05) + (C3×0.10) + (C4×0.10) + (C5×0.05) + (C6×0.05) + (C7×0.30)`
...
| 기준 | 가중치 | 이유 |
|---|---|---|
| 1. 산업 및 역할 적합성 | **35%** | 산업이 맞지 않으면 나머지 기준이 의미 없음 |
| 7. 의사결정권 | **30%** | 전환율에 가장 직접적 영향 |
| 2. 비즈니스 활동 규모 | **5%** | 플랫폼 ROI와 직결 |
| 3. 회사 규모 | **10%** | Self-serve 온보딩 가능 여부 결정 |
| 4. 성장 신호 | **10%** | 타이밍 신호 |
| 5. 기술 성숙도 | **5%** | 플랫폼 교체 장벽 판단 |
