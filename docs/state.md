...
| `reports` | Annotated[list[Report], reports_reducer] | 생성된 리포트 목록 (병렬 fan-out 누적, `None` 입력 시 초기화) |
| `reports_folder_link` | str | Google Drive 리포트 폴더 링크 |
| `custom_outreach_report_link` | str | 개인화 아웃리치 리포트 링크 |
| ~~`personalized_email`~~ | ~~str~~ | 미사용 (dead code) — reports 리스트 방식으로 대체됨 |
| ~~`interview_script`~~ | ~~str~~ | 미사용 (dead code) — reports 리스트 방식으로 대체됨 |
...
### `Annotated[list[Report], reports_reducer]` — 병렬 fan-out 누적 + 초기화 패턴

```python
def reports_reducer(existing: list, update):
    if update is None:
        return []              # None이면 초기화 (새 리드 시작 시)
...
- **초기화 메커니즘:** `fetch_linkedin_profile_data`가 `{"reports": None}`을 반환하면 reducer가 `[]`를 돌려줘 새 리드 처리 시작 시 이전 리드의 리포트를 초기화합니다.
- 기존 `operator.add`는 `None` 처리 불가로 인해 커스텀 `reports_reducer`로 교체되었습니다.
