...
**동작:**
- `state["leads_ids"]`가 있으면 해당 ID 목록만 조회
- 없으면 `status_filter="NEW"` 기준으로 전체 조회
...
**반환:** `{"current_lead": ..., "company_data": ..., "reports": None, "reports_folder_link": "", "custom_outreach_report_link": ""}`
> `reports: None` 반환은 `reports_reducer`를 통해 리포트 목록을 `[]`로 초기화하는 신호입니다. `reports_folder_link`와 `custom_outreach_report_link`도 함께 빈 문자열로 초기화하여 새 리드 처리 시작 시 이전 리드 데이터를 완전히 지웁니다.
...
**점수 파싱:**
1. `FINAL SCORE: X.X` 형식 우선 파싱 (정규식)
2. 해당 형식 없으면 마지막 숫자 fallback 추출
...
4. Google Docs에 저장 후 공유 링크 반환
   - 폴더명: `f"[agent] {lead.name.strip()} ({lead.id})"` (예: `"[agent] John Smith (rec123abc)"`)
...
- Google Docs 저장 시 폴더명: `f"[agent] {lead.name.strip()} ({lead.id})"` — `generate_custom_outreach_report`와 동일한 폴더 사용
