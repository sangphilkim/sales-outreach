...
**동작 방식:** 검색 결과 중 `linkedin.com/in`이 포함된 첫 번째 링크를 반환합니다.
...
| `is_company` 값 | 사용 URL |
|----------------|----------|
| `False` (개인) | `fresh-linkedin-profile-data.p.rapidapi.com/get-linkedin-profile` |
...
- `extract_linkedin_url_base`는 단순 URL 매칭 방식으로 정확도가 낮을 수 있습니다.
- `extract_linkedin_url` (LLM 방식)이 더 정확하지만 API 비용이 발생합니다.
