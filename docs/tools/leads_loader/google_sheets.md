...
### `_col_index_to_letter(index)` *(static 내부 메서드)*

0-based 컬럼 인덱스를 Google Sheets 열 문자로 변환합니다. A-Z 이후 AA, AB 등 두 글자 열도 정상 처리합니다.
...
- `_col_index_to_letter()` 헬퍼를 통해 AA, AB 등 26개 초과 컬럼도 정상 처리됩니다.
